...
static int oplus_3rd_pps_action_volt_change(struct oplus_pps_chip *chip)
{
	...
	if (chip->target_charger_volt < charger_volt_min) {
		chip->count.target_vbus_over++;
		chip->target_charger_volt = charger_volt_min;
		pps_err("3rd pps target_charger_volt(%d) too low, times:%d, charger_volt_min:%d\n",
				chip->target_charger_volt, chip->count.target_vbus_over, charger_volt_min);
		if (chip->count.target_vbus_over > PPS_3RD_ASK_VOLT_OVER_CNT) {
			chip->count.target_vbus_over = 0;
			chip->pps_status = OPLUS_PPS_STATUS_CHECK;
		}
	} else {
		chip->count.target_vbus_over = 0;
...
{
	struct oplus_pps_chip *chip = &g_pps_chip;
	if (!chip || !chip->ops || !chip->pps_support_type || chip->pps_stop_status)
		return;
